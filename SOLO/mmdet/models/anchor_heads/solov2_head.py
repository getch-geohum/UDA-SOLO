...
class SOLOv2Head(nn.Module):
    ...
    def loss(self,
             ...
             img_metas,
             cfg,
             gt_bboxes_ignore=None,return_raw=False):
        ...
        # ins
        ins_labels = [torch.cat([ins_labels_level_img.cuda()
        ...
        ins_ind_labels = [
            torch.cat([ins_ind_labels_level_img.flatten().cuda()
        ...
        for input, target in zip(ins_pred_list, ins_labels):
            ...
            instant_pred.append(input)  # change
            instant_ref.append(target)  # change
        loss_ins = torch.cat(loss_ins).mean()
        loss_ins = loss_ins * self.ins_loss_weight
        inst_tensor_pred = torch.cat(instant_pred)  # change
        instant_tensor_ref = torch.cat(instant_ref) # change
        instances = dict(pred = inst_tensor_pred, ref = instant_tensor_ref)  # change

        # cate
        cate_labels = [
            torch.cat([cate_labels_level_img.flatten().cuda()
        ...
        category = dict(pred = flatten_cate_preds, ref = flatten_cate_labels)  # change

        loss_cate = self.loss_cate(flatten_cate_preds, flatten_cate_labels, avg_factor=num_ins + 1)
        if return_raw:
            return instances, category  # change
        else:
