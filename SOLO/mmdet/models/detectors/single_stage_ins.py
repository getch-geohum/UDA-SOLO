...
class SingleStageInsDetector(BaseDetector):
    ...
    def extract_feat(self, img):
        x = self.backbone(img)
        #print(f'the shape of backbone is --> {len(x)},{[xx.shape for xx in x]}')
        if self.with_neck:
            x_1 = self.neck(x)
        return x, x_1

    def forward_dummy(self, img):
        y, x = self.extract_feat(img)
        outs = self.bbox_head(x)
        return outs

    def forward_train(self, img,return_feature='resnet'):
        assert return_feature in ['resnet', 'fpn'], 'the feature output type {return_feature} not known'
        y, x = self.extract_feat(img) # changed one
        outs = self.bbox_head(x)
        ...
        if self.with_mask_feat_head and return_feature == 'resnet':
            return y, outs, mask_feat_pred
        elif self.with_mask_feat_head and return_feature == 'fpn':
            return x, outs, mask_feat_pred
